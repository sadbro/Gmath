...
class Metrics:
    ...
    def sin(x, iter=80, points=4, unit="rad"):
        ...
        if unit == "rad":
            ...
            raise MetricError("Unidentified angle unit: {}".format(unit))
    ...
    @staticmethod
    def derivate(xs: list, ys:list, show=False, label:str=label):

        width = (xs[-1] - xs[0])/len(xs)
        dys = []

        for i, _ in enumerate(ys):
            try:
                dy = (ys[i + 1] - ys[i])/width
                dys.append(dy)
            except IndexError:
                pass
        ...
        if show:
            ...
            plt.plot(xs, dys, label="derivative")
            plt.legend(loc="best")
            plt.grid(True)
            plt.show()
    ...
    @staticmethod
    def derivate_n(xs:list, ys:list, n:int, label:str = "function"):

        width = (xs[-1] - xs[0])/len(xs)
        ys0 = ys
        plt.plot(xs, ys0, label=label)

        for i in range(1, n+1):
            ys0 = Metrics.derivate(xs, ys0)[1]
            plt.plot(xs, ys0, label="derivative - {}".format(i))
